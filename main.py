...
def ddl(update,bot,message,url,file_name='',thread=None,jdb=None):
    ...
    if not downloader.stoping:
        if file:
            ...
            bot.editMessageText(message,'❗️Enlaces de MEGA no soportados❗️')
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if '/ban' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    jdb.remove(user)
                    jdb.save()
                    msg = '🔒 @'+user+' a sido Baneado'
                    bot.sendMessage(update.message.chat.id,msg)
                except:
                    bot.sendMessage(update.message.chat.id,'❗️Error en el comando❗️')
            else:
                bot.sendMessage(update.message.chat.id,'❗️No tiene permisos de Administrador❗️')
            return
        if '/getdb' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
        ...
        if '/zip' in msgText:
            ...
            if getUser:
                try:
                   ...
                   jdb.save_data_user(username,getUser)
                   jdb.save()
                   msg = '❕Tamaño de Zips cambiados a '+ sizeof_fmt(size*1024*1024) 
                   bot.sendMessage(update.message.chat.id,msg)
                except:
                   bot.sendMessage(update.message.chat.id,'❗️Error en el comando❗️')    
                return
        ...
        #         bot.editMessageText(message,'❌Error y Causas🧐\n1-Revise su Cuenta\n2-Servidor Desabilitado: '+client.path)
            ...
            #    bot_token = '5549441898:AAFfjG0qvhBsKmRYbWOonkiBWdliq3woxT8'
